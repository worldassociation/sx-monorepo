...
<template>
  <UiWrapperInput :definition="inline ? omit(definition, ['title']) : definition" :error="error" :dirty="dirty"
    class="relative mb-[14px] w-auto">
    <Combobox v-slot="{ open }" v-model="inputValue" as="div" nullable>
      <Float :adaptive-width="inline ? false : true" strategy="fixed" placement="bottom-start"
        :offset="gap && inline ? gap : 0">
        <div :class="{
          relative: inline
        }">
          <ComboboxButton class="w-full" as="div" :data-value="content" :class="{
            sizer: inline
          }">
            <ComboboxInput class="s-input !flex items-center justify-between !mb-0" :class="{
              '!rounded-b-none': !gap && open,
              'h-[42px] min-w-11': inline
            }" :size="'1'" autocomplete="off" :placeholder="definition.examples?.[0]"
          ...
          <div v-if="inline" class="absolute top-[-7px] bg-skin-bg px-1 left-2.5 text-sm text-skin-text leading-4">
            {{ definition.title }}
          </div>
        </div>
          ...
          <div class="max-h-[208px] overflow-y-auto">
            ...
            <ComboboxOption v-for="item in filteredOptions" v-slot="{ selected, disabled, active }"
              ...
              <li class="flex items-center justify-between px-3" :class="{
                ...
                <span class="w-full py-2 text-skin-link truncate" :class="{
                  'opacity-40': disabled,
