...
<template>
  <div class="flex gap-3 justify-between items-center w-full py-2 px-3 rounded border text-[17px]" :class="{
    ...
    <slot />
    <button v-if="dismissible" type="button" class="text-skin-link opacity-50 hover:opacity-100" @click="emit('close')">
      <IH-x />
    </button>
  </div>
</template>
