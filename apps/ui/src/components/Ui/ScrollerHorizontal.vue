...
const props = withDefaults(
  ...
  { withButtons: false, gradient: false, stickyOffset: 72 }
);

const { isVisible, isMobile } = useScrollVisibility();

const stickyTopClass = computed(() => {
  if (!isMobile.value) return `top-[${props.stickyOffset}px]`;
  return isVisible.value ? `top-[${props.stickyOffset}px]` : 'top-0';
});
...
<template>
  <div class="relative sticky z-40 transition-[top] duration-300 bg-skin-bg" :class="stickyTopClass">
    <div class="relative">
      ...
      <div ref="containerRef" class="overflow-x-auto no-scrollbar"
        @scroll="e => e.target && handleScroll(e.target as HTMLElement)">
        <div data-no-sidebar-swipe>
          <slot />
        </div>
      </div>
      <button v-if="gradient && (!withButtons || canScrollTabs.canScrollRight)" type="button" tabindex="-1"
        :disabled="!withButtons" class="bg-gradient-to-l from-skin-bg right-0 top-[1px] bottom-[1px] absolute z-10"
        :class="{
