...
<template>
  <button type="button" v-bind="$attrs"
    ...
    <img v-if="imgUrl" alt="" :src="imgUrl" class="size-full object-cover group-hover:opacity-80" :class="{
      'opacity-80': isUploadingImage
    }" />
    <SpaceCover v-else-if="props.space?.cover" :space="props.space"
      class="pointer-events-none group-hover:opacity-80" />
    <UserCover v-else-if="props.user?.cover" :user="props.user"
  ...
  <input ref="fileInput" type="file" accept="image/jpg, image/jpeg, image/png" class="hidden"
