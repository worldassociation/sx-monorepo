  ...
  <UiDropdown :gap="gap" :placement="placement">
      ...
      <slot name="button">
        <button type="button"
          ...
          <template v-if="currentItem">
            <div v-if="currentItem.indicator" class="size-[8px] rounded-full" :class="currentItem.indicator" />
            <component :is="currentItem.component" v-else-if="currentItem.component"
    ...
    <template #items>
      <UiDropdownItem v-for="item in items" :key="item.key" v-slot="{ active, disabled }">
        <button type="button" class="flex items-center gap-2" :class="{ 'opacity-80': active, 'opacity-40': disabled }"
          @click="model = item.key">
          <div v-if="item.indicator" class="size-[8px] rounded-full" :class="item.indicator" />
