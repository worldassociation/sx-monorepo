...
<template>
  <div class="max-w-[592px] s-box">
    ...
    <div class="mb-3">
      ...
      <UiMarkdown v-if="previewEnabled" class="px-3 py-2 mb-3 border rounded-lg min-h-[200px]"
        :body="stripHtmlTags(form.statement)" />
      <UiComposer v-else v-model="form.statement" :definition="STATEMENT_DEFINITION" :error="formErrors.statement" />
    </div>
    <div class="flex items-center justify-between space-x-2.5">
      <UiButton class="w-full" @click="$emit('close')">Cancel</UiButton>
      <UiButton primary class="w-full" :disabled="!formValidated || Object.keys(formErrors).length > 0"
        :loading="sending" @click="handleSubmit">
        Save
      </UiButton>
    </div>
  </div>
