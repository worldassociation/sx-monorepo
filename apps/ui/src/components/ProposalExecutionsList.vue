  ...
  <div v-for="execution in executions" :key="`${execution.networkId}:${execution.safeAddress}`"
    ...
    <a :href="getTreasuryExplorerUrl(execution.networkId, execution.safeAddress) ||
      ...
      <div class="flex-1 leading-[22px] overflow-hidden">
        <h4 class="text-skin-link truncate" v-text="execution.safeName || shorten(execution.safeAddress)" />
        <div class="text-skin-text text-[17px] truncate" v-text="getExecutionName(proposal.network, execution.strategyType) ||
    ...
    <div class="flex justify-between items-center border-y pr-3">
      ...
      <UiTooltip v-if="execution.strategyType === 'ReadOnlyExecution'" title="Export transactions">
        <button type="button" class="hover:text-skin-link p-2" @click="downloadExecution(execution)">
          <IS-arrow-down-tray />
        </button>
      </UiTooltip>
    </div>
