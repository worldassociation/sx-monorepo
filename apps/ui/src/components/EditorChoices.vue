...
<template>
  <div class="s-base" :class="{
    's-error': showError
  }">
    <h4 class="eyebrow mb-2.5">Choices</h4>
    <div class="flex flex-col space-y-3">
      <Draggable v-model="proposal.choices" v-bind="{ animation: 200 }" handle=".handle" class="flex flex-col gap-2"
        item-key="index">
        <template #item="{ index }">
          <div>
            <div class="flex items-center rounded-lg bg-skin-border h-[40px] gap-2.5 pl-2.5">
              <div v-if="proposal.type !== 'basic'" class="text-skin-text handle cursor-grab">
                <IC-drag />
              </div>
              <div v-else class="mt-1.5">
                <div class="shrink-0 rounded-full choice-bg inline-block size-[18px]" :class="`_${index + 1}`">
                  <IH-check v-if="index === 0" class="text-white size-[14px] mt-0.5 ml-0.5" />
                  <IH-x v-else-if="index === 1" class="text-white size-[14px] mt-0.5 ml-0.5" />
                  <IH-minus-sm v-else-if="index === 2" class="text-white size-[14px] mt-0.5 ml-0.5" />
                </div>
              </div>
              <div class="grow">
                <input :ref="el => (choices[index] = el)" v-model.trim="proposal.choices[index]" type="text"
                  :maxLength="definition.items[0].maxLength"
                  class="w-full h-[40px] py-[10px] bg-transparent text-skin-heading"
                  :placeholder="getPlaceholderText(index)" @keyup.enter="handlePressEnter(index)"
                  @keydown.delete="e => handlePressDelete(e, index)" />
              </div>
              <UiButton v-if="shouldHaveDeleteButton(index)"
      ...
      <UiButton v-if="proposal.type !== 'basic'" class="w-full flex items-center justify-center space-x-1"
