...
<template>
  ...
  <UiMessage type="info" :learn-more-link="'https://docs.snapshot.org/user-guides/spaces/sub-spaces'">
  ...
  <div class="s-box my-3">
    <UiInputString v-model="parent" :loading="isParentLoading" :definition="PARENT_SPACE_DEFINITION"
      :error="formErrors.parent ?? parentValidationError" />
    <UiInputString v-model="child" :loading="isChildLoading" :definition="CHILD_DEFINITION"
      :error="formErrors.child ?? childValidationError" />
    <UiButton v-if="children.length < CHILDREN_LIMIT" :disabled="!!formErrors.child || !addSubSpaceButtonEnabled"
      class="w-full" @click="addChild">
      Add space
    </UiButton>
  </div>
  <div class="flex flex-wrap gap-2">
    <div v-for="(space, i) in children" :key="space" class="flex items-center gap-2 rounded-lg border px-3 py-2 w-fit">
      <span>{{ space }}</span>
  ...
  <div class="s-box">
    <UiInputString v-model="termsOfServices" :definition="TERMS_OF_SERVICES_DEFINITION"
      :error="formErrors.termsOfServices" />
  </div>
  <h4 class="eyebrow mb-2 font-medium mt-4">Custom domain</h4>
  <UiMessage type="info" :learn-more-link="'https://docs.snapshot.org/spaces/add-custom-domain'">
  ...
  <div class="s-box mt-3">
    <UiInputString v-model="customDomain" :definition="CUSTOM_DOMAIN_DEFINITION" :error="formErrors.customDomain" />
    <UiSwitch v-model="isPrivate" title="Hide space from homepage" />
  </div>
  <h4 class="eyebrow mb-2 font-medium mt-4">Danger zone</h4>
  <div class="flex flex-col md:flex-row gap-3 md:gap-1 items-center border rounded-md px-4 py-3">
    ...
    <UiButton :disabled="!isController" class="w-full md:w-auto flex-shrink-0" :class="{
  ...
  <teleport to="#modal">
    <ModalDeleteSpace :open="isDeleteSpaceModalOpen" :space-id="spaceId" @confirm="emit('deleteSpace')"
