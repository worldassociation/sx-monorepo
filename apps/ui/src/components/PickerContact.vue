...
<template>
  <div>
    ...
    <template v-else>
      ...
      <button v-for="contact in filteredContacts" :key="contact.address" type="button"
        class="w-full px-3 py-2.5 border-b last:border-0 flex justify-between" @click="emit('pick', contact.address)">
        <div class="flex items-center max-w-full">
          ...
          <div class="flex flex-col ml-3 leading-5 overflow-hidden text-left">
            ...
            <div class="text-[17px] text-ellipsis overflow-hidden" v-text="shortenAddress(contact.address)" />
          </div>
        </div>
      </button>
    </template>
