...
<template>
  <div v-if="votingPower.status === 'error'" class="flex flex-col gap-3 items-start" v-bind="$attrs">
    <UiAlert type="error">
  ...
  <UiAlert v-else-if="action === 'vote' && !votingPower.canVote" type="error" v-bind="$attrs">
    You do not have enough voting power to vote.
  </UiAlert>
  <UiAlert v-else-if="action === 'propose' && !votingPower.canPropose" type="error" v-bind="$attrs">
