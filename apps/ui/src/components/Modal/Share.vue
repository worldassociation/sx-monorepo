...
<template>
  <UiModal :open="open" class="text-skin-heading" @close="emit('close')">
    <div class="flex flex-col space-y-3 px-4 py-5 text-center items-center text-skin-text">
      ...
      <div class="flex flex-col space-y-1 leading-6">
        <h4 class="font-semibold text-skin-heading text-lg" v-text="messages.title" />
      ...
      <div class="flex flex-col">
        ...
        <div class="flex space-x-2">
          <a v-for="(socialNetwork, i) in SOCIAL_NETWORKS" :key="i"
            class="rounded-full leading-[100%] border text-skin-link size-[46px] flex items-center justify-center"
            :title="`Share on ${socialNetwork.name}`" :href="getShareUrl(socialNetwork.id, 'vote', shareable)"
            target="_blank">
    ...
    <div v-if="txId" class="flex items-center justify-center p-4 pt-3 -mt-2">
      <a :href="network.helpers.getExplorerUrl(txId, 'transaction')" target="_blank">
        View on explorer
