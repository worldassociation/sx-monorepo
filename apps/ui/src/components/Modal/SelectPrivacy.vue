...
<template>
  <UiModal :open="open" @close="$emit('close')">
    ...
    <div class="p-4 flex flex-col gap-2.5">
      <UiSelector v-for="type in votingTypes" :key="type" :is-active="initialState === type"
        @click="handleSelect(type)">
        <div>
          <h4 class="text-skin-link" v-text="PRIVACY_TYPES_INFO[type].label" />
          <div v-if="PRIVACY_TYPES_INFO[type].description" v-text="PRIVACY_TYPES_INFO[type].description" />
        </div>
      </UiSelector>
    </div>
  </UiModal>
