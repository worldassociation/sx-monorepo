...
<template>
  <UiModal :open="open" @close="emit('close')">
    <template #header>
      <h3>Add strategy</h3>
      <div v-if="!isLoading" class="flex items-center border-t px-2 py-3 mt-3 -mb-3">
    ...
    <div class="p-4 flex">
      ...
      <div v-else class="flex flex-col flex-1 gap-3 overflow-hidden">
        <button v-for="strategy in filteredStrategies" :key="strategy.address" type="button"
          class="flex flex-col gap-2 p-[20px] border rounded-md" @click="handleStrategySelected(strategy)">
          <div class="flex items-center gap-1 w-full">
            <span class="text-md text-skin-link font-semibold leading-5 truncate">
              {{ strategy.name }}
            </span>
