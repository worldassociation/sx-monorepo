...
<template>
  <UiModal :open="open" @close="$emit('close')">
    ...
    <div v-else-if="votingPower">
      <MessageVotingPower class="p-4" :voting-power="votingPower" @fetch-voting-power="$emit('fetchVotingPower')" />
      <div v-for="(strategy, i) in votingPower.votingPowers" :key="i" class="py-3 px-4 border-b last:border-b-0">
        <div class="flex justify-between">
          <a :href="network.helpers.getExplorerUrl(strategy.address, 'strategy')" target="_blank" class="truncate"
        ...
        <div class="flex justify-between">
          <div v-if="strategy.token" class="flex items-center gap-2">
            <a :href="(network.constants.STORAGE_PROOF_STRATEGIES_TYPES?.includes(
              strategy.address
            )
                ? baseNetwork
                : network
              ...
              " target="_blank" class="flex items-center text-skin-text">
            ...
            <a v-if="strategy.swapLink" :href="strategy.swapLink" target="_blank"
