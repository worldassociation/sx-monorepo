...
<template>
  <UiModal :open="open" @close="$emit('close')">
    ...
    <div v-else class="s-box p-4">
      <UiInputAddress v-model="form.delegatee" :definition="DELEGATEE_DEFINITION" :error="formErrors.delegatee"
        @pick="showPicker = true" />
    </div>
    <template v-if="!showPicker" #footer>
      <UiButton class="w-full" :loading="sending" :disabled="Object.keys(formErrors).length > 0" @click="handleSubmit">
        Confirm
      </UiButton>
    </template>
  </UiModal>
