...
<template>
  <div v-if="!delegation.apiUrl || delegation.apiType === 'split-delegation'"
  ...
  <template v-else>
    ...
    <div class="text-left table-fixed w-full">
      <div class="bg-skin-bg border-b sticky z-40 flex w-full font-medium space-x-3 px-4 transition-[top] duration-300"
        ...
        <button type="button"
          ...
          <IH-arrow-sm-down v-if="sortBy === 'tokenHoldersRepresentedAmount-desc'" class="shrink-0" />
          <IH-arrow-sm-up v-else-if="sortBy === 'tokenHoldersRepresentedAmount-asc'" class="shrink-0" />
        </button>
        <button type="button"
      ...
      <template v-else>
        <div v-if="loaded && (delegates.length === 0 || failed)" class="px-4 py-3 flex items-center space-x-1">
          <IH-exclamation-circle class="shrink-0" />
        ...
        <UiContainerInfiniteScroll :loading-more="loadingMore" @end-reached="handleEndReached">
          <div v-for="(delegate, i) in delegates" :key="i" class="border-b flex space-x-3 px-4">
            <AppLink :to="{
              name: 'space-user-statement',
              params: {
                space: spaceKey,
              ...
              <div class="flex grow sm:grow-0 sm:shrink-0 items-center w-[190px] py-3 gap-x-3 leading-[22px] truncate">
                ...
                <div class="flex flex-col truncate">
                  <h4 class="text-skin-link truncate" v-text="delegate.name || shorten(delegate.user)" />
                  <div class="text-[17px] text-skin-text truncate" v-text="shorten(delegate.user)" />
                </div>
              </div>
              <div
                class="hidden sm:flex items-center grow text-[17px] overflow-hidden leading-[22px] text-skin-heading">
                <div v-if="delegate.statement" class="line-clamp-2 max-h-[44px]" v-text="shorten(removeMarkdown(delegate.statement.statement), 250)
                  " />
              </div>
              <div class="hidden md:flex shrink-0 w-[120px] flex-col items-end justify-center leading-[22px] truncate">
              ...
              <div class="w-[150px] flex flex-col sm:shrink-0 text-right justify-center leading-[22px] truncate">
              ...
              <UiDropdown>
                ...
                <template #items>
                  ...
                  <UiDropdownItem v-slot="{ active }">
                    ...
                    }" class="flex items-center gap-2" :class="{ 'opacity-80': active }">
                      <IH-user-circle />
                      View profile
                    </AppLink>
                  </UiDropdownItem>
                  <UiDropdownItem v-slot="{ active }">
                    <a :href="getExplorerUrl(delegate.user, 'address') || ''" target="_blank"
                      class="flex items-center gap-2" :class="{ 'opacity-80': active }">
    ...
    <teleport to="#modal">
      <ModalDelegate :open="delegateModalOpen" :space="space" :delegation="delegation"
        :initial-state="delegateModalState" @close="delegateModalOpen = false" />
    </teleport>
  </template>
</template>
