...
<template>
  <div class="flex flex-col gap-3">
    <div v-for="(member, i) in model" :key="member.address"
      class="flex justify-between items-center rounded-lg border px-4 py-3 text-skin-link">
      <div class="flex flex-col">
        <a :href="network.helpers.getExplorerUrl(member.address, 'address')" target="_blank"
          class="flex items-center text-skin-text leading-5">
          <UiStamp :id="member.address" type="avatar" :size="18" class="mr-2 !rounded" />
          {{ shorten(member.address) }}
          <IH-arrow-sm-right class="-rotate-45" />
        </a>
      </div>
      <div class="flex gap-3">
        <UiDropdown :disabled="member.role === 'admin' && !isAbleToChangeAdmins">
          <template #button>
            <button type="button" class="flex items-center gap-2" :class="{
          ...
          <template #items>
            <UiDropdownItem v-slot="{ active }">
              <button type="button" class="flex items-center gap-2 lg:min-w-[200px]" :disabled="!isAbleToChangeAdmins"
                :class="{
            ...
            <UiDropdownItem v-slot="{ active }">
              <button type="button" class="flex items-center gap-2 lg:min-w-[200px]" :class="{ 'opacity-80': active }"
                @click="changeMemberRole(i, 'moderator')">
                Moderator
                <UiTooltip :title="'Able to manage the space\'s proposals and create proposals'">
                  <IH-question-mark-circle />
                </UiTooltip>
              </button>
            </UiDropdownItem>
            <UiDropdownItem v-slot="{ active }">
              <button type="button" class="flex items-center gap-2 lg:min-w-[200px]" :class="{ 'opacity-80': active }"
                ...
                Author
                <UiTooltip :title="'Able to create proposals without having to go through proposal validation'">
                  <IH-question-mark-circle />
                </UiTooltip>
              </button>
            </UiDropdownItem>
        ...
        <button type="button" :disabled="member.role === 'admin' && !isAbleToChangeAdmins" :class="{
          'opacity-40 cursor-not-allowed':
            member.role === 'admin' && !isAbleToChangeAdmins
  ...
  <teleport to="#modal">
    <ModalAddMembers :open="modalOpen" :is-controller="isController" :current-members="model" @close="modalOpen = false"
      @add="addMembers" />
  </teleport>
</template>
