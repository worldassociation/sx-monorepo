  ...
  <div class="rounded-lg border px-4 py-3 text-skin-link leading-[18px]">
    <div class="flex justify-between items-center">
      ...
      <div v-if="!readOnly" class="flex gap-3">
        ...
        " type="button" @click="$emit('editStrategy', strategy)">
          <IH-pencil />
        </button>
        <button type="button" @click="$emit('deleteStrategy', strategy)">
          <IH-trash />
        </button>
        <a v-if="!hasAddress" :href="network.helpers.getExplorerUrl(strategy.address, 'strategy')" target="_blank"
          class="text-skin-link">
          <IH-information-circle />
        </a>
      </div>
    </div>
    <a v-if="hasAddress" :href="network.helpers.getExplorerUrl(strategy.address, 'contract')" target="_blank"
      class="flex items-center text-skin-text leading-5 mt-1">
      <UiStamp :id="strategy.address" type="avatar" :size="18" class="mr-2 !rounded" />
      {{ shorten(strategy.address) }}
      <IH-arrow-sm-right class="-rotate-45" />
    </a>
    <div class="flex flex-col gap-2 mt-3 empty:mt-0">
      <div v-if="strategyNetworkDetails" class="flex gap-2 justify-between items-center overflow-hidden">
        <span class="font-medium">Network</span>
        <div class="flex gap-1 items-center">
          <img :src="getUrl(strategyNetworkDetails.logo) || undefined" class="size-3 rounded-full" />
          <span class="text-skin-text truncate">
            {{ strategyNetworkDetails.name }}
          </span>
        </div>
      </div>
      <div v-if="strategy.params.symbol" class="flex gap-2 justify-between items-center">
