  ...
  <div class="py-12">
    <div class="border-y mb-4">
      <UiContainer class="!max-w-screen-lg flex items-center space-x-3">
        <IH-search />
        <input v-model="q" type="text" placeholder="Search for integrations"
    ...
    <UiContainer class="!max-w-screen-lg space-y-4">
      ...
      <div v-else-if="q">
        ...
        <div v-if="results.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
      ...
      <div v-else>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
          <AppsListItem v-for="(app, i) in apps.filter(({ featured }) => featured)" :key="i" :app="app" />
        </div>
        <div v-for="(category, i) in categories" :key="i">
          <UiLink :count="apps.filter(app => category === app.category).length" :text="category" class="inline-block" />
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
            <AppsListItem v-for="(app, j) in apps.filter(app => category === app.category)" :key="j" :app="app" />
          </div>
        </div>
      </div>
    </UiContainer>
