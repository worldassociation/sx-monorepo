...
<template>
  <UiScrollerHorizontal with-buttons gradient="xxl" :sticky-offset="72">
    <div class="flex px-4 space-x-3 bg-skin-bg border-b min-w-max">
      <AppLink v-for="tab in tabs.filter(tab => tab.visible)" :key="tab.id" :to="{
        name: 'space-settings',
        params: { space: route.params.space, tab: tab.id }
      }" type="button" class="scroll-mx-8" @focus="handleTabFocus">
  ...
  <div v-else class="space-y-4 pb-8" :class="{
    ...
    <div v-show="activeTab === 'profile'">
      <FormSpaceProfile :id="space.id" :space="space" :form="form" @errors="v => (formErrors = v)" />
    </div>
    <UiContainerSettings v-if="activeTab === 'delegations'" title="Delegations"
      description="Delegations allow users to delegate their voting power to other users.">
      <FormSpaceDelegations v-model="form.delegations" :network-id="space.network"
        :limit="isOffchainNetwork ? 1 : undefined" />
    </UiContainerSettings>
    <UiContainerSettings v-if="activeTab === 'treasuries'" title="Treasuries"
      description="Treasuries are used to manage the funds of the space.">
      <FormSpaceTreasuries v-model="form.treasuries" :network-id="space.network"
        :limit="isOffchainNetwork ? 10 : undefined" />
    </UiContainerSettings>
    <UiContainerSettings v-else-if="activeTab === 'strategies'" title="Strategies"
      description="Strategies are sets of conditions used to calculate user's voting power.">
      <FormSpaceStrategies v-model:snapshot-chain-id="snapshotChainId" v-model:strategies="strategies"
        :network-id="space.network" :is-ticket-valid="isTicketValid" :space="space" />
    </UiContainerSettings>
    <FormStrategies v-if="activeTab === 'authenticators'" v-model="authenticators" unique :network-id="space.network"
      :available-strategies="network.constants.EDITOR_AUTHENTICATORS" title="Authenticators"
    ...
    <FormValidation v-else-if="activeTab === 'proposal-validation'" v-model="validationStrategy"
      :network-id="space.network" :available-strategies="network.constants.EDITOR_PROPOSAL_VALIDATIONS"
      :available-voting-strategies="network.constants.EDITOR_PROPOSAL_VALIDATION_VOTING_STRATEGIES
    ...
    <FormStrategies v-else-if="activeTab === 'voting-strategies'" v-model="votingStrategies" :network-id="space.network"
      :available-strategies="network.constants.EDITOR_VOTING_STRATEGIES" title="Voting strategies"
    ...
    <UiContainerSettings v-else-if="activeTab === 'proposal'" title="Proposal"
      ...
      <FormSpaceProposal v-model:proposal-validation="proposalValidation" v-model:guidelines="guidelines"
        v-model:template="template" :network-id="space.network" :snapshot-chain-id="snapshotChainId" :space="space"
        @update-validity="v => (hasProposalErrors = !v)" />
    </UiContainerSettings>
    <UiContainerSettings v-else-if="activeTab === 'voting'" title="Voting"
      ...
      <FormSpaceVoting v-model:voting-delay="votingDelay" v-model:min-voting-period="minVotingPeriod"
        v-model:max-voting-period="maxVotingPeriod" v-model:quorum-type="quorumType" v-model:quorum="quorum"
        v-model:voting-type="votingType" v-model:privacy="privacy" v-model:vote-validation="voteValidation"
        v-model:ignore-abstain-votes="ignoreAbstainVotes" :snapshot-chain-id="snapshotChainId" :space="space"
        @update-validity="v => (hasVotingErrors = !v)" />
    </UiContainerSettings>
    <UiContainerSettings v-else-if="activeTab === 'members'" title="Members"
      description="Members have different roles and permissions within the space.">
      <FormSpaceMembers v-model="members" :network-id="space.network" :is-controller="isController"
        :is-admin="isAdmin" />
    </UiContainerSettings>
    <UiContainerSettings v-else-if="activeTab === 'labels'" title="Labels"
      description="Labels are used to categorize proposals.">
      <FormSpaceLabels v-model="form.labels" />
    </UiContainerSettings>
    <UiContainerSettings v-else-if="activeTab === 'execution'" title="Execution(s)"
      ...
      <div class="space-y-3">
        <FormStrategiesStrategyActive v-for="strategy in executionStrategies" :key="strategy.id" read-only
          :network-id="space.network" :strategy="strategy" />
      </div>
    </UiContainerSettings>
    <UiContainerSettings v-else-if="activeTab === 'controller'" title="Controller"
      ...
      <div class="flex justify-between items-center rounded-lg border px-4 py-3 text-skin-link">
        <div class="flex flex-col">
          <a :href="network.helpers.getExplorerUrl(controller, 'contract')" target="_blank"
            class="flex items-center text-skin-text leading-5">
            <UiStamp :id="controller" type="avatar" :size="18" class="mr-2 !rounded" />
            {{ shorten(controller) }}
            <IH-arrow-sm-right class="-rotate-45" />
          </a>
        </div>
        <button type="button" :disabled="!isController" :class="{
      ...
      <teleport to="#modal">
        <ModalChangeController :open="changeControllerModalOpen" :initial-state="{ controller }"
          @close="changeControllerModalOpen = false" @save="handleControllerSave" />
      </teleport>
    </UiContainerSettings>
    <UiContainerSettings v-show="activeTab === 'advanced'" title="Advanced">
      <FormSpaceAdvanced v-model:parent="parent" v-model:children="children" v-model:terms-of-services="termsOfServices"
        v-model:custom-domain="customDomain" v-model:is-private="isPrivate" :network-id="space.network"
        :space-id="space.id" :is-controller="isController" @delete-space="handleSpaceDelete" @update-validity="(valid, resolved) => {
          hasAdvancedErrors = !valid;
          isAdvancedFormResolved = resolved;
        }
          " />
    </UiContainerSettings>
    <UiToolbarBottom v-if="
    ...
    " class="px-4 py-3 flex flex-col xs:flex-row justify-between items-center">
      <h4 class="leading-7 font-medium truncate mb-2 xs:mb-0" :class="{ 'text-skin-danger': error }">
      ...
      <div class="flex space-x-3">
        ...
        <UiButton v-if="!error" :loading="saving" primary @click="handleSettingsSave">
          Save
        </UiButton>
      </div>
    </UiToolbarBottom>
  ...
  <teleport to="#modal">
    <ModalTransactionProgress :open="saving" :network-id="space.network" :messages="{
      approveTitle: 'Confirm your changes',
      successTitle: 'Done!',
      successSubtitle: 'Your changes were successfully saved'
    }" :execute="executeFn" @confirmed="reloadSpaceAndReset" @close="saving = false" />
  </teleport>
</template>
