...
<template>
  <div>
    <div class="relative h-[156px] md:h-[140px] mb-[-86px] md:mb-[-70px] top-[-1px]">
    ...
    <div class="px-4">
      <div class="mb-4 relative">
        <AppLink :to="{ name: 'space-overview' }">
          <SpaceAvatar :space="space" :size="90" class="relative mb-2 border-4 border-skin-bg !rounded-lg -left-1" />
        </AppLink>
        <div class="flex items-center">
          <h1 v-text="space.name" />
          <UiBadgeVerified v-if="!isWhiteLabel" class="ml-1 top-0.5" :verified="space.verified" :turbo="space.turbo" />
        </div>
        <div class="mb-3 flex flex-wrap gap-x-1 items-center">
          ...
          <template v-if="!isWhiteLabel && space.parent">
            ...
            <AppLink :to="{
              ...
              <SpaceAvatar :space="space.parent" :size="22" class="rounded-md" />
              <span>{{ space.parent.name }}</span>
            </AppLink>
          </template>
        </div>
        <div v-if="space.about" class="max-w-[540px] text-skin-link text-md leading-[26px] mb-3 break-words"
    ...
    <template v-if="!isWhiteLabel && space.children.length">
      ...
      <UiScrollerHorizontal gradient="md">
        <div class="px-4 py-3 flex gap-3 min-w-max">
          <SpacesListItem v-for="child in space.children" :key="child.id" :space="child" :show-about="false"
            class="w-[240px]" />
        </div>
      </UiScrollerHorizontal>
    </template>
    <div>
      <ProposalsList title="Proposals" :loading="!proposalsRecord?.summaryLoaded" :limit="PROPOSALS_LIMIT - 1"
        :proposals="proposalsRecord?.summaryProposals ?? []" :route="{
